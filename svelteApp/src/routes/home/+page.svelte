<script>
    ...
    import Waves_3 from '../../components/Waves_3.svelte';
    import {getUserCount} from '$lib/users_api';
    ...
    onMount(async () => {
        try {
            const studentPromise = getUserCount('student');
            const teacherPromise = getUserCount('teacher');

            studentPromise.then(count => tweetedStudentCount.set(count));
            teacherPromise.then(count => tweetedTeacherCount.set(count));
        } catch (error) {
    ...
    const pages = [
        ...
        {name: 'Delogare', icon: 'fas fa-sign-out-alt', href: '/'}
    ];
</script>
    ...
    <div class="relative h-full bg-indigo-600">
        ...
        <div class="container mx-auto px-4 py-8 relative z-10">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-4">
                {#each pages as page}
                    <a href={page.href}
                       class="flex flex-col items-center justify-center p-4 bg-white bg-opacity-20 rounded-lg hover:bg-opacity-30 transition-colors duration-200 aspect-square">
