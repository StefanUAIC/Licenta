                        ...
                        <tbody class="bg-transparent divide-y divide-gray-200 gap-y-px">
                            {#each classesBodySliced as classItem, index}
                                <tr>
                                    ...
                                    <td class="px-6 py-1 whitespace-nowrap class-name bg-white rounded-l-full cursor-pointer hover:highlight"
                                        on:click={() => goToClass(classItem.id)}>
                                        {classItem.name}
                                    </td>

                                    <td class="px-6 py-4 whitespace-nowrap bg-white rounded-r-full text-right cursor-pointer hover:highlight"
                                        on:click={() => goToClass(classItem.id)}>27 students
                                    </td>
                                    {#if role === 'teacher'}
...
<style>
    ...
    /* Stil pentru hover simultan */
    .highlight {
        background-color: #e0e0e0;
    }

    /* Adăugăm o regulă specială pentru a face hover simultan */
    table tbody tr:hover td:nth-child(2),
    table tbody tr:hover td:nth-child(3) {
        background-color: #e0e0e0; /* Sau orice efect de hover dorești */
    }
</style>
